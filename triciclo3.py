...
def triciclos3(sc,files):
    rdd = sc.parallelize([])
    for file in files: #Ahora hacemos el analogo a get_rdd_distinct edges en el bucle, para guardar la información del fichero a cada nodo
        file_rdd = sc.textFile(file).\
            map(lambda x : get_edges(x,file)).\
            filter(lambda x: x is not None).\
            distinct()
        rdd = rdd.union(file_rdd) #Los unimos a un mismo rdd y todo igual que en triciclos1 y triciclos2
